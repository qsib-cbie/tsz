...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    impl Value for XyzValue {}

    let tmp_dir = TempDir::new("can_get_real_file").unwrap();
    // copy over ./0001-1686168000000000-9223372037005771051-index.bin in tmp_dir
    let current_dir = std::env::current_dir().unwrap();
    let src =
        current_dir.join("tests/data/0001-1686168000000000-9223372037005771051-index.bin");
    let dst = tmp_dir
        .path()
        .join("0001-1686168000000000-9223372037005771051-index.bin");
